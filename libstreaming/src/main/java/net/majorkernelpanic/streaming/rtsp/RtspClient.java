...
//        sendRequestRecord();
        ...
        Response response = sendRequestOption();

        if (response != null && response.status == 200) {
            response = sendRequestDescribe();
        } else {
            throw new IOException("sendRequestOption return fail " + response.status);
        }

        if (response != null && response.status == 200) {
            response = sendRequestSetup();
        } else {
            throw new IOException("sendRequestDescribe return fail " + response.status);
        }

        if (response != null && response.status == 200) {
            Log.d(TAG, "u can sen PLAY");
            sendRequestPLAY(response.headers.get("Session"));
        } else {
            throw new IOException("sendRequestSetup return fail " + response.status);
        }
     ...
     */
    private void sendRequestPLAY(String session) throws IOException {
     ...
     */
    private Response sendRequestDescribe() throws IOException {
        String body = "Accept: application/sdp \r\n";
        ...
        mOutputStream.write(request.getBytes("UTF-8"));
        mOutputStream.flush();
        return Response.parseResponseWithBody(mBufferedReader);
    }
     ...
     */
    private Response sendRequestSetup() throws IllegalStateException, SocketException, IOException {
//        for (int i = 0; i < 2; i++) {
        int i = 0;
...
//        }
     ...
     * get server provide method
     */
    private Response sendRequestOption() throws IOException {
        String request = "OPTIONS rtsp://" + mParameters.host + ":" + mParameters.port + mParameters.path + " RTSP/1.0\r\n" + addHeaders();
        Log.i(TAG, request);
        mOutputStream.write(request.getBytes("UTF-8"));
        mOutputStream.flush();
        return Response.parseResponse(mBufferedReader);
    }
    ...
    static class Response {
        ...
        public static Response parseResponse(BufferedReader input) throws IOException, IllegalStateException, SocketException {
            ...
            while ((line = input.readLine()) != null) {
                Log.i(TAG, "l: " + line.length() + ", c: " + line);
                if (line.length() > 3) {
        ...
        public static Response parseResponseWithBody(BufferedReader input) throws IOException {
            Response response = new Response();
            Matcher matcher;
            String line;
            StringBuffer mHeader = new StringBuffer();
            ...
            while ((line = input.readLine()) != null) {
                ...
                if (!isBody) {
                    isBody = TextUtils.isEmpty(line) && readCount > 1;
                }
                if (line.startsWith("Content-Length:")) {
                    line = line.replace(" ", "");
                    bodyLength = Integer.parseInt(line.substring(line.indexOf(":") + 1));
                }
                if (isBody && line.length() > 2) {
                    mBody.append(line + "\r\n");
                } else {
                    if (readCount == 1) {
                        matcher = regexStatus.matcher(line);
                ...
                if (bodyLength > 0 && mBody.toString().getBytes().length == bodyLength) {
                    response.bodies = mBody.toString();
                    break;
                }
            }
//            Log.i(TAG, "parse header:" + mHeader.toString() + ",length:" + mHeader.toString().getBytes().length +
//                    "\n,parse body:" + mBody.toString() + ",length:" + mBody.toString().getBytes().length +
